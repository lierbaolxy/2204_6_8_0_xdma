...
/* Compatibility for kernels >= 6.1 where swait_event_interruptible_timeout was removed */
...
static void engine_service_perf(struct xdma_engine *engine, u32 desc_completed)
{
	...
	if (engine->xdma_perf) {
		...
		if (engine->status & XDMA_STAT_DESC_STOPPED) {
			 ...
			 */
#if	0 /* force standard wait queues; swait API causes preempt_count imbalance on 6.8+ */
			swake_up_all(&engine->xdma_perf_wq);
#else
			...
			struct sg_table *sgt, bool dma_mapped, int timeout_ms)
{
	...
	while (nents) {
		...
		/* Release desc_lock before waiting to avoid scheduling while atomic */
		spin_unlock(&engine->desc_lock);
				...
				int dir, struct pci_dev *pdev)
{
	...
	for (i = 0; i < npages; i++, sg = sg_next(sg)) {
		...
		if (pdev) {
			dma_addr_t bus = dma_map_page(&pdev->dev, pg, 0, PAGE_SIZE,
							dir);
                	if (unlikely(dma_mapping_error(&pdev->dev, bus))) {
